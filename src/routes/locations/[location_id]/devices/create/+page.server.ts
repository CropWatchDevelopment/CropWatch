...
export const actions: Actions = {
    default: async ({ request, locals, params, fetch }) => {
        ...
        if (!values.dev_eui) {
            return fail(400, { error: 'Device EUI is required.', ...values });
        }

        const normalizedDevEui = normalizeDevEui(values.dev_eui);
        if (normalizedDevEui.length !== 16) {
            return fail(400, {
                error: 'Device EUI must contain exactly 16 hexadecimal characters.',
                ...values
            });
        }

        if (!values.name) {
            return fail(400, { error: 'Device name is required.', ...values });
        }

        const typeId = readOptionalInteger(values.type);
        if (!typeId) {
            return fail(400, { error: 'Device type id is required.', ...values });
        }

        const locationId = readOptionalInteger(values.location_id);
        if (!locationId) {
            return fail(400, { error: 'Location id is required.', ...values });
        }
        ...
        try {
            ...
            const message = readApiMessage(
                error,
                'Failed to create device. The current API docs still mark this endpoint as not implemented.'
            );
