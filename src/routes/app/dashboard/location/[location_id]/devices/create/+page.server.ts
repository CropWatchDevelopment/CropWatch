...
export const load: PageServerLoad = async (event) => {
	...
	try {
		...
		const deviceTypeResults = await deviceTypeRepository.findAll();
		const deviceTypes = deviceTypeResults.map((type) => ({
			id: type.id,
			name: type.name ?? ''
		}));
		const permissionTypes: Array<[string, PermissionLevel]> = Object.entries(PermissionLevel)
			.filter(([key]) => isNaN(Number(key))) // Get string keys from enum
			.map(([key, value]) => [key, value as PermissionLevel] as [string, PermissionLevel]);
...
export const actions: Actions = {
	createDevice: async (event) => {
		...
		if (!name || !devEui || userPermissionsData === null) {
			// lat/long can be optional depending on DB
		...
		try {
			...
			if (!createdDevice) {
				...
				return fail(500, {
					message: 'Failed to create device record. The service returned no device.'
				});
			}

			// Add current user as Admin owner in cw_device_owners
			await deviceRepository.addUserToDevice(
				createdDevice.dev_eui,
				currentUserId,
			...
			for (const perm of parsedUserPermissions) {
				if (perm.permissionLevelId !== PermissionLevel.Disabled) {
					// Only add if not disabled
					await deviceRepository.addUserToDevice(
						createdDevice.dev_eui,
						perm.userId,
						perm.permissionLevelId
					);
				}
			}
			...
			return fail(500, {
				message:
					'Failed to create device or set permissions. ' +
