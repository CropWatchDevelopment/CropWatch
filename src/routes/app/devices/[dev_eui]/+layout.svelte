    ...
    const makePdf = async () => {
        try {
            ...
            const response = await fetch(
                `/api/v1/devices/${$page.params.dev_eui}/reports/?month=${selectedMonth.toISOString()}`
            );
            ...
            pdfMake.createPdf(docDefinition).download(`${moment(selectedMonth).format('yyyy-MM')}-report.pdf`);
