		...
		return () => {
			...
			return async ({ result, update }: { result: ActionResult; update: () => Promise<void> }) => {
				...
				if (result.type === 'success') {
					success($_('Password updated successfully! You can now log in with your new password.'));
					goto('/auth/login');
				} else if (result.type === 'failure' && result.data?.error) {
					error($_(result.data.error));
				} else {
					error($_('Failed to update password'));
				}
			};
		};
	}
...
<svelte:head>
	<title>{$_('Update Password')} | CropWatch</title>
</svelte:head>
...
>
	...
	>
		<h1 class="mb-6 text-center text-2xl font-bold">{$_('Update Password')}</h1>
		...
		{:else}
			...
			<form method="POST" use:enhance={handleSubmit()} class="space-y-4">
				<div>
					<label for="newPassword" class="mb-1 block text-sm font-medium"
                   ...
                   focus:ring-2 focus:outline-none dark:border-gray-700 dark:bg-gray-800"
					/>
					<p class="mt-1 text-xs text-gray-500">{$_('Minimum 8 characters required')}</p>
				</div>

				<div>
					<label for="confirmPassword" class="mb-1 block text-sm font-medium"
						>{$_('Confirm New Password')}</label
					>
					<input
						type="password"
						id="confirmPassword"
