...
export const GET: RequestHandler = async ({ params, fetch, locals: { supabase, safeGetSession } }) => {
    ...
    const notice = data.data.filter(item => (item.temperature >= -18.1 && item.temperature < 15.1)).length;
    const warning = data.data.filter(item => (item.temperature >= -15.1 && item.temperature < 0)).length;
