<script lang="ts">
	...
	import { useAppState, useFilters } from '$lib/data/AppState.svelte';
	import { onMount, onDestroy } from 'svelte';
	import {
		fetchDataTableRows,
		startDeviceRealtime,
		type DataTabKey
	} from '$lib/data/SourceOfTruth.svelte';
	...
	const selectedFacilityId = $derived(filters.getFacility());
	const selectedLocationId = $derived(filters.getLocation());
	...
	const tabColumnsByKey: Record<DashboardTabKey, ColumnConfig[]> = {
		air: [
			...
			{ key: 'lastSeen', label: 'Last seen', type: 'datetime', sortable: true },
			{
				...
				sortable: true,
				href: (item: Device) => `/locations/location/${item.locationId}?prev=${$page.url.pathname}`
			},
			{
				key: 'Actions',
				label: 'Actions',
	...
	const getSessionTokens = () => {
		return sessionToTokens($page.data.session);
	};
...
<div class="flex h-screen min-h-0 overflow-hidden bg-slate-950 text-slate-50">
	...
	<main class="flex min-h-0 flex-1 flex-col overflow-hidden bg-slate-950">
		...
		<header class="flex-none border-b border-slate-800 bg-slate-950/90 backdrop-blur">
			...
			>
				<div class="hidden w-full md:flex flex-wrap items-center gap-3 text-slate-400">
					...
					<span class="flex items-center gap-1 mr-5 {offline == 0 ? 'text-green-300' : 'text-rose-300'}">
