...
type LocationWithCount = {
	cw_devices?: DeviceWithSensorData[] | null;
};
...
export function isSoilSensor(device: DeviceWithSensorData): boolean {
	...
	return (
		...
		Boolean(device.latestData && 'moisture' in device.latestData)
	);
}
...
export function getLocationActiveStatus(
	...
	deviceActiveStatus: Record<string, boolean>
): { activeDevices: DeviceWithSensorData[]; allActive: boolean; allInactive: boolean } {
	...
	const allActive = locationDevices.every((device) => isDeviceActive(device, deviceActiveStatus));

	const allInactive = locationDevices.every(
		(device) => !isDeviceActive(device, deviceActiveStatus)
	);
