...
export async function fetchHistoricalWeather(startDate: Date, endDate: Date, latitude: number = 34.6937, longitude: number = 135.5023): Promise<any[]> {
    console.log(`Fetching weather data from ${startDate.toISOString().split('T')[0]} to ${endDate.toISOString().split('T')[0]}`);
...
export function getWeatherCacheStats(): { entries: number; oldestEntry: string | null; newestEntry: string | null } {
    const cache = getWeatherCache();
