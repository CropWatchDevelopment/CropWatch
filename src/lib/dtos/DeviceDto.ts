...
export function toDeviceDto(device: Device): DeviceDto {
	return {
		...
		long: device.long ?? undefined,
		upload_interval: device.upload_interval ?? undefined
	};
}
...
export function toDeviceDetailDto(
	...
	const detailDto: DeviceDetailDto = {
		...
		installed_at: device.installed_at ?? undefined,
		battery_changed_at: device.battery_changed_at ?? undefined,
		warranty_start_date: device.warranty_start_date ?? undefined
	};

	if (deviceType) {
		detailDto.deviceType = {
			id: deviceType.id,
			name: deviceType.name ?? undefined,
			manufacturer: deviceType.manufacturer ?? undefined,
			model: deviceType.model ?? undefined
		};
	}
