	...
	use:enhance={() => {
		return async ({ result, update }) => {
			if (result.type === 'success' && result.data && result.data.success) {
				...
				if (result.data.data && (result.data.data as any).id) {
					await goto(`/app/dashboard/location/${(result.data.data as any).id}`);
				}
			} else {
...
>
	<div class="grid gap-6 lg:grid-cols-[minmax(0,380px)_minmax(0,1fr)]">
		<div class="border-border bg-card space-y-6 rounded-xl border p-6 shadow-sm">
			<div class="space-y-2">
				<label for="name" class="text-text text-sm font-semibold">
					{$_('Location Name')}
				</label>
				<TextInput
					id="name"
