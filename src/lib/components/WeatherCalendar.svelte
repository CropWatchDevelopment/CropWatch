<script lang="ts">
	...
	fetchHistoricalWeather()
		.then((data) => {
			...
			const mappedEvents = data.map((entry) => ({
				...
				title: `${entry.weather.maxtemp_c}°/${entry.weather.mintemp_c}°C ${entry.weather.totalprecip_mm}mm`,
				display: 'auto',
				extendedProps: {
				...
				html: `
					<div style="display:flex; flex-direction:column; align-items:center; font-size:10px; padding:2px;">
						...
						<div style="text-align:center; line-height:1.1;">
							${event.extendedProps.maxTemp}°/${event.extendedProps.minTemp}°C<br>
							${event.extendedProps.precipitation}mm
						</div>
					</div>
				`
			};
	...
	const options = $derived({
		view: 'dayGridMonth',
		date: new Date(2025, 3, 1), // April (month index 3 = April)
...
<section>
	<div>
		...
		<p>Current calendar view: April 2025</p>
	</div>
	<Calendar 
		plugins={[DayGrid]} 
		{options}
