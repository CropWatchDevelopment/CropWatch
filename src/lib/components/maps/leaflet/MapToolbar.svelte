<script lang="ts">
	// Svelte 5 syntax - using callback props instead of createEventDispatcher
	interface Props {
		onclickeye?: (visible: boolean) => void;
		onclicklines?: (visible: boolean) => void;
		onclickreset?: () => void;
	}
	
	let { onclickeye, onclicklines, onclickreset }: Props = $props();
