<script lang="ts">
	...
	$: filteredDevices = devices.filter((device) =>
		data.cw_locations.cw_device_locations.some(
			(locationDevice) => locationDevice.dev_eui === device.dev_eui
		)
	);
						...
						<div class="my-1 mr-2 border-r-2">
							<div class="flex flex-col text-center text-base">
								<div class="flex flex-row justify-left">
									<b class="text-sm ml-4 text-slate-800"
										>{getDeviceByDevEui(device.dev_eui)?.cw_devices?.name ?? 'un-named'}</b
									>
								</div>
						...
						{#each Object.keys(convertObject(device)) as dataPointKey, index}
							{#if device[dataPointKey] != null}
								<div class="py-1">
									...
									<!-- Add a border if not the last element -->
