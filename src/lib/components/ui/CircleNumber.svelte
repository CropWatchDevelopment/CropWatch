<script lang="ts">
    ...
    $: bgColor = computeColor(value);

    function computeColor(temp: number) {
...
<div class="bg-[#34393f] bg-opacity-50 rounded-xl py-5 flex justify-center text-surface-100 my-3">
    <div
        class="h-60 w-60 rounded-full bg-gradient-to-r from-[#375270] to-[#2108b4] flex justify-center items-center"
	...
	<div class="h-56 w-56 rounded-full bg-[#343f34] bg-opacity-80 flex justify-center items-center">
        <div class="h-56 w-56 rounded-full bg-[#343f34] bg-opacity-80 flex justify-center items-center">
            <div class="text-center space-y-4">
                <p class="text-5xl text-center">{value}<span class="text-4xl text-[#A3A3A3]">{notation}</span></p>
            </div>
        </div>
		</div>
    </div>
