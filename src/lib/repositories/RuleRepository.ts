...
export class RuleRepository extends BaseRepository<Rule, number> {
	...
	async findActive(): Promise<Rule[]> {
		...
		if (error) {
			this.errorHandler.handleDatabaseError(error, 'Error finding active rules');
		}
	...
	override async create<I>(entity: I): Promise<Rule> {
		...
		if (error) {
			this.errorHandler.handleDatabaseError(error, 'Error creating rule');
		}
	...
	async createCriteria(criteria: RuleCriteriaInsert): Promise<RuleCriteria> {
		...
		if (error) {
			this.errorHandler.handleDatabaseError(error, 'Error creating rule criteria');
		}
	...
	override async update<U>(id: number, entity: U): Promise<Rule | null> {
		...
		if (error) {
			...
			this.errorHandler.handleDatabaseError(error, `Error updating rule with ID: ${id}`);
		}
	...
	async deleteCriteria(id: number): Promise<boolean> {
		const { error } = await this.supabase.from(this.criteriaTable).delete().eq('id', id);

		if (error) {
			this.errorHandler.handleDatabaseError(error, `Error deleting rule criteria with ID: ${id}`);
		}
