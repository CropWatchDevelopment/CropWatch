...
export class LocationRepository extends BaseRepository<Location, number> {
   ...
   */
  async checkUserPermission(locationId: number, userId: string, requiredLevel: number = 3): Promise<boolean> {
    const { data, error } = await this.supabase
      .from('cw_location_owners')
   ...
   */
  async addUserToLocation(locationId: number, userId: string, permissionLevel: number): Promise<void> {
