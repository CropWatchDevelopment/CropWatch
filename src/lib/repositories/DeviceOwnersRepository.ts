...
export class DeviceOwnersRepository extends BaseRepository<DeviceOwner, number> {
	...
	async findAll(): Promise<DeviceOwner[]> {
		const { data, error } = await this.supabase.from(this.tableName).select('*').order('id');

		if (error) {
			this.errorHandler.handleDatabaseError(error, 'Error finding all device owners');
		}
	...
	async getDevicesByUserId(userId: string): Promise<DeviceOwner[]> {
      ...
      `
		...
		if (error) {
			this.errorHandler.handleDatabaseError(error, `Error finding devices by user ID: ${userId}`);
		}
	...
	override async create<I>(entity: I): Promise<DeviceOwner> {
		...
		if (error) {
			this.errorHandler.handleDatabaseError(error, 'Error creating device owner');
		}
	...
	override async update<U>(id: number, entity: U): Promise<DeviceOwner | null> {
		...
		if (error) {
			this.errorHandler.handleDatabaseError(error, `Error updating device owner with ID: ${id}`);
		}
